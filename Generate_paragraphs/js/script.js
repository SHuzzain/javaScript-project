...
const paragraphContainer = document.getElementsByClassName('gen-para')[0]


const generateParagraph = () => {
   let inputValTxtToNum = inputVal.value
    inputVal.value == "" ? inputValTxtToNum = inputVal.getAttribute("placeholder") : inputValTxtToNum = inputVal.value;

   for(i=1;i<=inputValTxtToNum;i++){
    const ran = [text[Math.floor(Math.random() * text.length)]]
    paragraphContainer.innerHTML += ran.map( item => `<p>${item} </p>`)
    
   }
}

inputVal.addEventListener("click", (even) => {
